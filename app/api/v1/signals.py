...
async def run_signals(body: SignalRunRequest, session: SessionDep) -> dict:
    ...
    # SQLite autoincrement workaround: BigInteger PKs are not auto-generated by
    # SQLite unless the column is declared with AUTOINCREMENT, which SQLAlchemy
    # does not emit for BigInteger.  Query the current max id once and track a
    # running counter across all instrument batches so ids never collide when
    # multiple instruments are persisted in a single request.
    if body.persist:
        _max = (await session.execute(select(func.max(Signal.id)))).scalar_one_or_none() or 0
    else:
        _max = 0
    _id_counter = _max
