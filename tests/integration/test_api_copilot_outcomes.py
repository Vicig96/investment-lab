...
@pytest.fixture
def outcome_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_outcomes_endpoint_returns_local_summary(client, outcome_paths):
    ...
    response = await client.post("/api/v1/copilot/outcomes", json={})

    assert response.status_code == 200
    data = response.json()
    assert data["summary"]["total_decisions_reviewed"] == 1
    assert data["summary"]["accepted_decisions"] == 1
    assert data["entries"][0]["entity"] == "SPY"
